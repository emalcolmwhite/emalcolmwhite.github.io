...
remDr$navigate("https://catalog.middlebury.edu/offerings/search/catalog/catalog%2FMCUG?term=term%2F202490&department=&keywords=&time_start=0&time_end=86400&type%5B%5D=genera%3Aoffering%2FLCT&type%5B%5D=genera%3Aoffering%2FSEM&search=Search")
...
titlesdata <- data.frame(matrix("bleh", nrow = 598, ncol = 6)) %>%
  rename("titles" = 1) %>%
  rename("distros" = 2) %>%
  rename("department" = 3) %>%
  rename("time" = 4) %>%
...
write_csv(titlesdata, "coursesNoSep3.csv")
write_csv(courses, "Fall24courses.csv")
