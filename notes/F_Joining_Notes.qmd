...
author: 'EMW'
date: '2023 -- may not be exactly what was covered in 2024'
format:
  html:
    toc: true
    code-overflow: wrap
...
portal_rodent %>% 
  drop_na(wgt) %>% 
  group_by(year) %>% 
  summarize(max_wgt = max(wgt)) %>% 
  ggplot(aes(x=year, y = max_wgt)) + 
  geom_line()
```
...
```{r}
portal_joined <- portal_rodent %>% 
  left_join(portal_species, by=c("species" = "speciescode"))
```

```{r}
portal_joined_inner <- portal_rodent %>% 
  inner_join(portal_species, by=c("species" = "speciescode"))
```
...
```{r}
portal_weather_joined <- portal_joined %>%
